...
class LoginVC: UIViewController {
    ...
    @IBAction func loginAction(sender: AnyObject)
    {
        ...
        Utilities.getUrlsFromCollections({ [weak self] (collectionsResponse, error) -> Void in
            ...
            // get the login token
            self?.getToken(tokenUrlAsString: collectionsResponse.tokenUrlString!,
                           username: username,
                           password: password,
                           ...
                           completionToken: {(responseToken, errorToken) -> Void in
                            ...
                            self?.getCurrentUserData(currentUserUrlString: collectionsResponse.currentUserString!,
    ...
    func getToken(tokenUrlAsString : String, username : String, password : String, client_id : String, completionToken:@escaping (_ responseToken:String?, _ errorToken:NSError?) -> ()) {
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest){ data, response, error in
            ...
            do
            {
                if let jsonToken = try JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? [String: AnyObject] {
    ...
    // get the user data
    func getCurrentUserData(currentUserUrlString : String, accessToken : String, completionCurrentUser:@escaping (_ responseUser:User?, _ errorUser:NSError?) -> ())
    {
        let currentUserUrl = URL(string: currentUserUrlString);
        
        let configuration = URLSessionConfiguration.default;
        let headers: [AnyHashable: Any] = ["Accept" as NSObject:"application/json" as AnyObject, "Authorization" as NSObject:"Bearer " + accessToken];
        configuration.httpAdditionalHeaders = headers;
        let session = URLSession(configuration: configuration)
        
        let currentUserTask = session.dataTask(with: currentUserUrl!) { (data, currentUserResponse, errorUserData) in
            // parse the currentUser data
            do
            {
                if let currentUserJson = try JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? [AnyHashable: AnyObject] {
